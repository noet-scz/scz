...
(function () {
  ...
  // query/publish идут ЧЕРЕЗ УЗЕЛ (AUDIT #1): связь и подпись в узле, зона не лезет в реле.
  function query(filters) {
    var list = Array.isArray(filters) ? filters : [filters];
    return fetch('/api/net/query', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(list) }).then(function (r) { return r.json(); }).then(function (a) { return Array.isArray(a) ? a : []; }).catch(function () { return []; });
  }
  function publish(tmpl) {
    return fetch('/api/net/publish', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(tmpl) }).then(function (r) { return r.json(); }).then(function (j) { if (j && j.error) throw new Error(j.error); return j; });
  }
